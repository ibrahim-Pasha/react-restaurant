  ...
  return (
    <div className="discount-contant">
      ...
      <div className="discount-type">
        <button
          onClick={() => typeButtonClick(2)}
          className={
            discountType === "percent" ? "discount-btn active" : "discount-btn"
          }
        >
          Yüzde
        </button>
        <button
          onClick={() => typeButtonClick(1)}
          className={
            discountType === "count" ? "discount-btn active" : "discount-btn"
          }
        >
          Tutar
        </button>
      ...
      {discountType === "count" ? (
        <span className="discount-total">
          Uygulanacak indirim: {totalValue.toFixed(2)}
        </span>
      ) : (
        <span className="discount-total">
          Uygulanacak indirim: %{totalValue.toFixed(2)}
      ...
      <div className="numeric-pad">
        ...
        <button className="numeric-btn" onClick={handleDecimalButtonClick}>
          <b>,</b>
        </button>
        <button className="numeric-btn" onClick={() => handleButtonClick(0)}>
          0
        </button>
        <button className="numeric-btn" onClick={handleDeleteButtonClick}>
          DEL
        </button>
      </div>
    </div>
