...
const PartialPayment: React.FC<partialPaymentprops> = ({ totalPrice, basket }) => {
    ...
    const calculateTotal = (value: string) => {
        ...
        else { setTotalValue(totalPrice); setEnteredValue(totalPrice.toString()); }
    ...
    return (
        <div className='pay-contant'>
            <div className='select-product-topay'>
                <div className='pay-headers'><b>Parçalı Öde</b></div>
                ...
                {basket.map((order) => {
                    if (paymnt.get(order)) {
                        ...
                        return (<div className='selected-orders' onClick={() => handleSelectOrder(order)}><div>{order.count}- {order.product.stok_adi_kisa}</div><div> ₺ {order.product.satis_fiyat * order.count}</div></div>
                        )
                    }
                    else {
                        console.log(false);

                        return (<div className='select-orders' onClick={() => handleSelectOrder(order)}><div>{order.count}- {order.product.stok_adi_kisa}</div><div> ₺ {order.product.satis_fiyat * order.count}</div></div>
                        )
                    }
                })}
            </div>
            <div className='enter-amount'>
                <div className='pay-headers'><b>Toplam</b> <span>₺{totalPrice}</span></div>
                <div>
                    <div className='selected-products'></div>
                    <span style={{ float: 'right' }}>Ödenecek Tutar : {totalValue.toFixed(2)}</span>
                    <div className="key-pad">
                        <div className="numeric-keypad">
                            <div className="numeric-buttons">
                                {
                                    [7, 8, 9, 4, 5, 6, 1, 2, 3].map((number) => {
                                        return (<button className='numeric-button' key={number} onClick={() => handleButtonClick(number)}>
                                            {number}
                                        </button>)
                                    })}
                                <button className='numeric-button' onClick={handleDecimalButtonClick}><b>,</b></button>
                                <button className='numeric-button' onClick={() => handleButtonClick(0)}>0</button>
                                <button className='numeric-button' onClick={handleDeleteButtonClick}>DEL</button>

                            </div>
                            <div className='pad-keys'>
                                <button className='pad-key'><b>,</b></button>
                                <button className='pad-key'>0</button>
                                <button className='pad-key'>DEL</button> </div>
                        </div>
                    </div>
            ...
            <div className='select-pay-type'>
                <div className='pay-headers'><b>Ödeme Tipleri</b></div>
